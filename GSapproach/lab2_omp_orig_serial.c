...
void SVD(int N, int P, float *D, float **U, float **SIGMA, float **V_T)
{
    ...
    printf("\n A0 = D_TXD: \n");
    //print_matrix(A, P, P, 0);
    float diff = matrix_similarity(D_T, P, N, product_two);
    printf("Reconstruction error (L2 norm): %f\n", diff);
    if (diff < TOLERANCE) {
        printf("SVD decomposition verified successfully!\n");
    } else {
        printf("SVD decomposition failed! Reconstruction error is too high.\n");
    }
    float *IntermediateResult = (float *)malloc(sizeof(float) * P * N); // P x N matrix
    multiply(*U, P, P, temp_sigma, P, N, IntermediateResult);

    float *ReconstructedMatrix = (float *)malloc(sizeof(float) * N * P); // N x P matrix
    multiply(IntermediateResult, P, N, *V_T, N, N, ReconstructedMatrix);

    printf("\n Reconstructed Matrix (U * SIGMA * V^T):\n");
    print_matrix(ReconstructedMatrix, N, P, 0);
    float similarity = matrix_similarity(D_T, N, P, ReconstructedMatrix);
    printf("L2 Difference between original and reconstructed matrix: %f\n", similarity);
